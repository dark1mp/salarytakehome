...
├── components/
│   ├── AdUnit.js                # Reusable AdSense ad component. Accepts `slot`, `hideOnMobile`, `layout`, and `format` props. Slots: 7756198179 (in-content), 1586479879 (top-of-page), 4603525459 (sidebar vertical 1), 9534353704 (sidebar vertical 2), 3520564956 (in-article blog ads), 1315738286 (sidebar vertical 3 — pay-rise results-aligned), 3284808016 (blog side-rail vertical — auto-rendered by LayoutWrapper when `narrow={true}`, visible at xl+ / 1280px+). For in-article ads use: `layout="in-article" format="fluid"`.
...
- **Narrow layout for blogs** — LayoutWrapper accepts a `narrow` prop (used on all blog pages and `/salary-breakdown`). At 2xl+ (1536px+) it centers content with `max-w-[900px]`. At xl (1280–1535px) it applies `pr-[192px]` to the content div to reserve space for the side-rail ad (which is absolutely positioned on the right). When modifying the narrow class, keep both rules: the xl padding reserves space when the viewport is too narrow for natural centering, and the 2xl reset (`2xl:pr-0`) avoids double-spacing once centering takes over. The side-rail ad slot `3284808016` is auto-rendered by LayoutWrapper on any narrow page — individual blog pages don't need to add it manually. Local preview: AdSense won't fill on localhost, so the `AdUnit` container collapses to zero height; swap in a placeholder div temporarily if you need to see position.
